...
import { collectImagesRecursive, isDirectory, isFile, isSupportedImagePath, } from './lib/discover.js';
import { getOutputRoot, ensureDir } from './lib/fs.js';
import { detectSquooshSupport, getMenuOptions, } from './lib/encoders.js';
import { processImages } from './lib/process.js';
function usage() {
...
async function loadSavedConfig(configPath) {
    try {
...
async function promptForSelection(options) {
    printMenu(options);
...
async function promptForQuality(selected) {
    const lossyEncoders = selected.filter((enc) => enc.lossy && enc.supportsQuality && enc.encoderType !== 'unsupported');
    if (lossyEncoders.length === 0) {
        return { default: 75, overrides: {} };
    }
    const defaultQualityAnswer = await inquirer.prompt([
        {
            type: 'input',
            name: 'quality',
...
async function main() {
    ...
    if (!squooshEnabled) {
        console.log(chalk.yellow('Squoosh WASM codecs are disabled in this Node version; using Sharp fallbacks where available.'));
    }
    let selection = [];
    let quality = null;
    ...
    if (args.yes) {
        if (savedConfig) {
            selection = savedConfig.selectedIds.filter((id) => menuOptions.some((option) => option.id === id));
