...
import { getSquooshEncodeOptions, } from './encoders.js';
import { buildOutputPath, copyFileAtomic, ensureDir, getOriginalExtension, getTempPath, writeFileAtomic, } from './fs.js';
function getQualityForEncoder(encoder, quality) {
...
async function encodeWithSharp(encoder, inputPath, outputPath, quality) {
...
async function encodeWithSquoosh(pool, encoder, inputBuffer, outputPath, quality) {
    ...
    const encoded = image.encodedWith[encoder.squooshCodec];
    if (!encoded?.binary) {
...
export async function processImages(params) {
    ...
    if (needsSquoosh) {
        try {
            ...
            unsupported = unsupported.concat(squooshEncoders.map((enc) => ({
                id: enc.id,
                label: enc.label,
                skipped: params.inputFiles.length,
            })));
    ...
    let completed = 0;
    const tasks = params.inputFiles.map((inputFile) => limit(async () => {
